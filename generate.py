...
def FrameXPWindow(image,title,active=True,close=1,maximize=1,minimize=1,question=0):
    ...
    IMAGE = Image.new("RGBA", (width,height), (0,0,0,0))
    IMAGE = put(IMAGE,resize(TopFrame,width,h(TopFrame),28,35,9,17,Resampling.NEAREST),0,0)
    IMAGE = put(IMAGE,LeftFrame.resize((w(LeftFrame),height-h(TopFrame)-3),Resampling.NEAREST),0,h(TopFrame),"00")
    IMAGE = put(IMAGE,RightFrame.resize((w(RightFrame),height-h(TopFrame)-3),Resampling.NEAREST),width,h(TopFrame),"20")
    IMAGE = put(IMAGE,cropx(BottomFrame,0,5),0,height,"02")
    IMAGE = put(IMAGE,cropx(BottomFrame,4,w(BottomFrame)-5).resize((width-10,h(BottomFrame)),Resampling.NEAREST),5,height,"02")
